...
class InsightWeightingPortfolioConstructionModel1(PortfolioConstructionModel):
    ...
    # def calsymbolweights(self,activeInsights):
    #     t_insights = activeInsights
    #     for model, group in groupby(insights, lambda x: x.SourceModel):
    #         group = list(group)
    #         # self.Algorithm.Debug(str(group))
    ...
    def DetermineTargetPercent(self, activeInsights):
        ...
        #     self.Algorithm.Debug('sourcemodel'+str(sourceModel))
        #     for symbol, g in groupby(sorted(list(f), key = lambda gg: gg.Symbol), lambda ggg: ggg.Symbol):
        #         self.Algorithm.Debug('symbol'+str(symbol))
        #         self.Algorithm.Debug('g'+str(g))
