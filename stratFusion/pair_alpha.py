...
class TechPairsAlphaModel(AlphaModel):
    ...
    def Update(self, algorithm: QCAlgorithm, slice: Slice) -> List[Insight]:
        ...
        if self.flag_u_v <= - self.exit or self.flag_v_u >= self.exit:
            ...
            insights.extend([
                Insight.Price(short, self.duration, InsightDirection.Up, weight=0, confidence = self.weight_v),
                Insight.Price(long, self.duration, InsightDirection.Down, weight=0, confidence = self.weight_v),
            ]) 
        ...
        if self.flag_u_v <= - self.enter and self.flag_v_u >= self.enter:
            insights.extend([
                Insight.Price(short, self.duration, InsightDirection.Down, weight=self.weight_v*self.beta, confidence = self.weight_v),
                Insight.Price(long, self.duration, InsightDirection.Up, weight=self.weight_v*self.beta * self.coef*algorithm.Portfolio[long].Price / algorithm.Portfolio[short].Price, confidence = self.weight_v),
            ])
        ...
        # Placing orders: if short is relatively underpriced, sell the pair
            ...
            insights.extend([
                Insight.Price(short, self.duration, InsightDirection.Up, weight=self.weight_v*self.beta, confidence = self.weight_v),
                Insight.Price(long, self.duration, InsightDirection.Down, weight=self.weight_v*self.beta*self.coef*algorithm.Portfolio[long].Price / algorithm.Portfolio[short].Price, confidence = self.weight_v ),
            ])
...
class HealthCarePairsAlphaModel(AlphaModel):
    ...
    Name = 'HealthCarePairsAlphaModel'
    window = {} # stores historical price used to calculate trading day's stock return
    coef = 0    # to be calculated: requested ratio of quantity_u / quantity_v
    day = 0     # keep track of current day for daily rebalance
    month = 0   # keep track of current month for monthly recalculation of optimal trading pair
    ...
    def Update(self, algorithm: QCAlgorithm, slice: Slice) -> List[Insight]:
        ...
        if self.count == 0:
            ...
            self.select_pair = [self.all_pair['BMY'],self.all_pair['ABBV']]
    ...
    #     history = algorithm.History(self.select_pair, 2, Resolution.Hour, dataNormalizationMode=DataNormalizationMode.ScaledRaw)
    #     history = history.close.unstack(level=0)
    #     for symbol in self.window:
    #         for i in range(2):
    #             self.window[symbol].append(history[str(symbol)][i])
...
class BasicMaterialPairsAlphaModel(AlphaModel):
    ...
    Name = 'BasicMaterialPairsAlphaModel'
    window = {} # stores historical price used to calculate trading day's stock return
    coef = 0    # to be calculated: requested ratio of quantity_u / quantity_v
    day = 0     # keep track of current day for daily rebalance
    month = 0   # keep track of current month for monthly recalculation of optimal trading pair
